...
@interface _SCXMLDictionaryParser : NSObject <NSXMLParserDelegate>
@end

@implementation _SCXMLDictionaryParser {
...
+ (NSDictionary *)dictionaryWithXML:(id)xml {
    _SCXMLDictionaryParser *parser = nil;
    if ([xml isKindOfClass:[NSString class]]) {
        parser = [[_SCXMLDictionaryParser alloc] initWithString:xml];
    } else if ([xml isKindOfClass:[NSData class]]) {
        parser = [[_SCXMLDictionaryParser alloc] initWithData:xml];
    }
